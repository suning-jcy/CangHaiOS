...
var (
	...
	BlockInUse        = errors.New("Block In Use")
)
...
type CacheOption struct {
	...
	MaxDirtytime int64
	//cache->OSS 之间的刷新工作队列长度，0表示默认值12
	FlushThreadSum int
	//单位毫秒，0表示默认自500ms
...
func Newopfile(nodehandle uint64, fullpath string, basefile *ossfs.FileInfo) *opfile {
	of := &opfile{
...
func (of *opfile) dirtysize(show bool) (size int64) {
	...
	for _, v := range of.blocks {
		...
		if show{
			glog.V(0).Infoln("size:",size," dirtysize:",v.dirtysize," off:",v.off," inflush:",v.isInFlush)
		}
	}
	return
}
...
func (of *opfile) removecachedata(data *cachedata)(hasdel bool,err error) {
	//先判断一下，避免必要的对cachedatalock的反复加锁
	hasdel = false
	data.RLock()
	if data.dirtysize != 0{
		data.RUnlock()
		return hasdel,NewDirtyDataErr
	}
	if atomic.LoadInt32(&(data.users)) != 0{
		data.RUnlock()
		return hasdel,BlockInUse
	}
	off:=data.off
	users:=data.users
...
type CacheDataMana struct {
	...
	driver      *SdossDriver
	filemap     map[*ossfs.FileInfo]*opfile
...
func NewCacheDataMana(driver *SdossDriver, opt *CacheOption) *CacheDataMana {
	cdm := &CacheDataMana{
		lrulist: list.New(),
		isstop:  false,
		driver:  driver,
		filemap: make(map[*ossfs.FileInfo]*opfile),
	}
	if opt != nil {
...
func (c *CacheDataMana) getopfile(finfo *ossfs.FileInfo) *opfile {
	c.filemaplock.RLock()
...
//每次移除缓存块的时候尝试一下
//需要of先被锁
func (c *CacheDataMana) tryremoveopfile(finfo *ossfs.FileInfo) bool {
...
//添加一个块到缓存中。
func (c *CacheDataMana) SetBlock(block *cachedata, nodehandle uint64, info *ossfs.FileInfo) bool {
	if c.isstop {
		return false
	}
	...
	//
	/*
	if temp := len(cachesolt) / Capacity; temp > 0 {
		switch temp {
		case 0:
			break
		case 1:
			glog.V(4).Infoln("SetBlock:",block.of.fullpath , "100-199 " )
		case 2:
			glog.V(4).Infoln("SetBlock:",block.of.fullpath , "200-299 " )
		case 3:
			glog.V(4).Infoln("SetBlock:",block.of.fullpath , "300-399 " )
		case 4:
			glog.V(4).Infoln("SetBlock:",block.of.fullpath , "400-499 " )
		case 5:
			glog.V(4).Infoln("SetBlock:",block.of.fullpath , "500 " )
		}
	}
	*/
...
func (c *CacheDataMana) Addopfile(nodehandle uint64, fullpath string, info *ossfs.FileInfo) *opfile {
	if c.isstop {
		return nil
	}
	c.filemaplock.Lock()
...
//触发某个文件的刷新
func (c *CacheDataMana) FlushFile(fileinfo *ossfs.FileInfo) error {
	start := time.Now()
	defer func() {
...
func waitClean2(of *opfile,wg *sync.WaitGroup,where string)(err error){
	var i =0
	var size int64= 0
	err = nil
	for {
		size = of.dirtysize(false)
		if size == 0{
			break
		}else{
...
func waitClean(of *opfile,wg *sync.WaitGroup,where string)(err error){
	...
	for {
		if of.err != nil {
			glog.V(0).Infoln("wait file:",of.fullpath," err:",of.err,"where:",where)
			break
		}
		size = of.dirtysize(false)
		if size == 0{
		...
		if lastSize == size {
			count++
			if count > 300000{//一个包5分钟还刷不下去就报超时失败
				size = of.dirtysize(true)
				err = errors.New("wait too long time")
				glog.V(0).Infoln("wait file:",of.fullpath," clean err:",err," size:",size,"where:",where)
				of.err = err
				break
			}
		}else{
...
func (c *CacheDataMana) flushFile(of *opfile) (int, error) {
	...
	for _, v := range res {
		...
		//err := c.upload(v)
		//if err != nil {
		//	glog.V(0).Infoln("filename:", v.of.fullpath, " off:", v.off, "upload err:", err)
		//	flusherr = err
		//	continue
		//}
	}
...
//确定刷新
//以实时为准，
func (c *CacheDataMana) SyncFile(fileinfo *ossfs.FileInfo, anywrite bool) (err error) {
	if !anywrite {
		return nil
	}
...
func (c *CacheDataMana)CreateEmptyFile(fullpath string)(err error){
	if strings.HasPrefix(fullpath,"/"){
		fullpath = strings.TrimPrefix(fullpath,"/")
	}
	_, err = c.driver.uploadnomalfile(fullpath, nil)
	if err != nil {
		glog.V(0).Infoln("Create Empty File:",fullpath,"err:", err)
	}
	return err
}
//只有写打开的文件才会有这个关闭动作
func (c *CacheDataMana) CloseFile(fileinfo *ossfs.FileInfo, fullpath string,anywrite bool) (err error) {
	of := c.getopfile(fileinfo)
	if of == nil {
		if anywrite && fileinfo.Size == 0{
			err = c.CreateEmptyFile(fullpath)
		}
		return err
	}
	if !anywrite {
...
//直接移除
func (c *CacheDataMana) Truncate(fileinfo *ossfs.FileInfo, size uint64) {
	start := time.Now()
	defer func() {
		if debug || time.Since(start) > 2*time.Second {
...
//删除文件时候使用
//提交+移除
func (c *CacheDataMana) RemoveFile(fileinfo *ossfs.FileInfo) {
	start := time.Now()
	defer func() {
		if debug || time.Since(start) > 2*time.Second {
...
func (c *CacheDataMana) RenameOpFile(fileinfo *ossfs.FileInfo,oldfullpath string,  newfullpath string) (err error) {
	c.filemaplock.Lock()
	defer c.filemaplock.Unlock()
...
func (c *CacheDataMana) GetNewBlockFromOss(of *opfile, off int64) (*cachedata, error) {
...
func (c *CacheDataMana) WriteAt(handle uint64, fullpath string, fileinfo *ossfs.FileInfo, data []byte, off int64) (err error) {
	//
	start := time.Now()
	defer func() {
		if debug || time.Since(start) > 2*time.Second {
	...
	for i := startidx; i <= endidx; i++ {
		...
		if tempblock == nil {
			glog.V(4).Infoln(" can not get block:", fullpath, " blockid:", i, " filesize:", fileinfo.Size, of.osssize)
			if blockoff == of.osssize {
				if debug {
					glog.V(0).Infoln(fullpath, "WriteAt: filesize is offset!,create new block. blockoff:", blockoff, " osssize:", of.osssize)
				}
				tempblock, err = c.GetNewBlock(of, blockoff)
				if err != nil {
					glog.V(0).Infoln("WriteAt:GetNewBlock error:", err, fileinfo.Size, of.osssize, fullpath)
					return err
				}
			} else if blockoff < of.osssize {
				if debug {
					glog.V(0).Infoln(fullpath, "WriteAt: get new block from oss. blockoff:", blockoff, " osssize:", of.osssize)
				}
				tempblock, err = c.GetNewBlockFromOss(of, blockoff)
				if err != nil {
					glog.V(0).Infoln("WriteAt:GetNewBlockFromOss error:", err, fileinfo.Size, of.osssize, fullpath)
					return err
				}
			} else {
				if debug {
					glog.V(0).Infoln(fullpath, "WriteAt: ", blockoff, " osssize:", of.osssize)
				}
				...
				if lastblock != nil && int64(len(lastblock.data)) == BlockSize {
					...
					glog.V(0).Infoln("WriteAt: WriteOutOfSize", fileinfo.Size, of.osssize, fullpath)
					return WriteOutOfSize
				}
			}
...
func (c *CacheDataMana) ReadAt(handle uint64, fullpath string, fileinfo *ossfs.FileInfo, size, off int64, data []byte) (ressize int64, err error) {
	...
	//endidx := int(math.Ceil(float64((off + size) / blocksize)))
	newoff := off
	left := size
	...
	for i := startidx; i <= endidx; i++ {
		...
		if tempblock == nil {
			...
			tempblock = of.GetBlock(blockoff)
			if tempblock == nil {
				glog.V(1).Infoln(" can not get block:", fullpath, " blockid:", i, " blockoff:", blockoff, " size:", of.osssize)
				if blockoff < of.osssize {
					tempblock, err = c.GetNewBlockFromOss(of, blockoff)
					if err != nil {
						<- of.blkchan[i]
						glog.V(0).Infoln("ReadAt:GetNewBlockFromOss error ", fullpath, " blockoff:", blockoff, " err:", err)
						return 0, err
					}
					atomic.AddInt32(&(tempblock.users), 1)
					c.SetBlock(tempblock, handle, fileinfo)
		...
		if int64(len(tempblock.data)) < (newoff - blockoff) {
			atomic.AddInt32(&(tempblock.users),-1)
			glog.V(0).Infoln("ReadAt: error ", fullpath, " blockoff:", blockoff, " err:data get from oss less than info'size")
			return 0, ReadOutOfSize
		}
...
func (c *CacheDataMana) flushBlock(offset int64, data []byte, of *opfile) error {
	...
	/*if of.basefile.Size < BlockSize {
		...
		glog.V(0).Infoln("file:",of.fullpath," flushBlock off:", offset," dirtysize:",len(data), " uploadnomalfile mode,", time.Since(start))
		of.cacheOnly = false
	} else*/
	{
		of.reopenlock.Lock()
		if of.uploadid == "" {
			...
			if of.cacheOnly {
				...
				if !of.isopend {
					ap, err := c.driver.reopen(of.fullpath)
					if err != nil {
						of.reopenlock.Unlock()
						glog.V(0).Infoln(" flushBlock  ", of.fullpath, " off:", offset, "flushBlock reopen err:", err)
						return err
					}
					if debug || time.Since(start) > 2*time.Second {
						glog.V(0).Infoln(" flushBlock  ", of.fullpath, " off:", offset, " reopen getuploadid timecost:", time.Since(start))
					}
					if ap.Ftype == "blockedFile" {
						//多出来的一般也没用，写的过程中，块肯定已经被替换了
					} else { //普通文件，直接把数据都写入缓存（缓存会以）
						//在超过blocksize就分块的情况下，不可能到这个分支。
						//todo
					}

					of.isopend = true
					of.uploadid = ap.Uploadid
					if of.uploadid == "" {
						if of.err != nil {
							of.reopenlock.Unlock()
							glog.V(0).Infoln("file:",of.fullpath," has writed failed.no more try.err:",of.err)
...
func (c *CacheDataMana) removeBlock(block *cachedata, f bool) error {
	...
	return c.removecachedata(block)
}
...
func (c *CacheDataMana) removecachedata(data *cachedata)(err error) {
	var hasdel bool
	hasdel,err = data.of.removecachedata(data)
...
func (c *CacheDataMana) refreshColdLoop() {
	...
	for _ = range timetick {
		...
		glog.V(1).Infoln(" cached block sum:",len(cachesolt), "getColdBlock sum:", len(datas), " now cached block in lru total sum:", c.lrulist.Len())
		for _, v := range datas {
...
func (c *CacheDataMana) upload(block *cachedata,idx int) (err error) {
	...
	if oldsize > block.of.osssize {
		block.of.osssize = oldsize
	}
...
func (f *flushWorker) start(idx int) {
	...
	for {
		...
		case <-f.trigger:
			...
			f.touchTrigger()
		case <-time.After(time.Duration(DirtyScanInterval) * time.Millisecond):
			//glog.V(4).Infoln(" routine idx:",idx," routine Sum:",FlushThreadSum)
			if f.isstop {
