...
var NotExist = RedisError("Key does not exist ")

// reads a bulk reply (i.e $5\r\nhello)
func readBulk(reader *bufio.Reader, head string) (interface{}, error) {
    var err error
    var data []byte
    ...
    case '$':
     ...
     case '*':
        ...
        for i := 0; i < size; i++ {
         ...
         return res,nil
    default:
        return nil, RedisError("Expecting Prefix '$' or ':'"+head)
    }
...
func (client *Client) Scan(cursor string,pattern string,limit string) (interface{}, error) {
    var args []string
    ...
    res, err := client.sendCommand("scan",args...)
...
func (client *Client) Get(key string) ([]byte, error) {
    res, e := client.sendCommand("GET", key)
    if e != nil  {
        return nil,e
    }
    if res == nil {
