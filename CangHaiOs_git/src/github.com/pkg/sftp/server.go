...
var sftpServerWorkerCount int = 8
...
func (p sshFxInitPacket) uid() uint64 { return 0 }
...
func (p sshFxpOpenPacket) respond(svr *Server) error {
	...
	return svr.sendPacket(sshFxpHandlePacket{p.Uid, p.ID, handle})
}
...
func statusFromError2(p ider, err error, errcode uint32) sshFxpStatusPacket {
	...
	if err != nil {
		...
		ret.StatusError.Code = errcode
		ret.StatusError.msg = err.Error()
		if err.Error() == "EOF" {
			//println("StatusErr", err.Error())
