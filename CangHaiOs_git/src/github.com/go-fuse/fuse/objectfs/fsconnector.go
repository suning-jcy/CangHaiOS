...
func (c *rawBridge) childLookup(out *fuse.EntryOut, n *Inode, context *fuse.Context) {
	n.Fileinfo().GetAttr((*fuse.Attr)(&out.Attr))

	//n.Node().GetAttr((*fuse.Attr)(&out.Attr), &ObjectFile{}, context)
...
func (c *FileSystemConnector) lookupUpdate(node *Inode) (id, generation uint64) {
	glog.V(0).Infoln("FileSystemConnector  lookupUpdate", node.Fileinfo().Name, node.handled)
	id, generation = c.inodeMap.Register(&node.handled)
	c.verify()
	glog.V(0).Infoln("FileSystemConnector  lookupUpdate end", node.Fileinfo().Name, node.handled)
	return
}
