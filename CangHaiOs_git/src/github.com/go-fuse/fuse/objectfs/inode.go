...
func newInode(fsNode Node, fileinfo *ObjectFile) *Inode {
	me := new(Inode)
...
//remove not use and not exist in new
//add not exist
//keep handle is not null and not exist in new,
//keep exist still
//used after opendir
func (n *Inode) ChildrenReset(children map[string]*Inode) {
	...
	for name, child := range children {
		...
		if _, ok := n.children[name]; !ok {
			...
			//refresh
			n.children[name].fileinfo = child.fileinfo
		}
	}
