...
func (c *FileSystemConnector) lookupMountUpdate(out *fuse.Attr, mount *fileSystemMount) (node *Inode, code fuse.Status) {
	code = mount.mountInode.Node().GetAttr(out, &ObjectFile{}, nil)
	if !code.Ok() {
...
func (c *FileSystemConnector) internalLookup(out *fuse.Attr, parent *Inode, name string, header *fuse.InHeader) (node *Inode, code fuse.Status) {
	glog.V(0).Infoln("FileSystemConnector  internalLookup")
	...
	if child != nil {
		glog.V(0).Infoln("child != nil 2")
		code = child.Fileinfo().GetAttr(out)
		//code = child.fsInode.GetAttr(out, &ObjectFile{}, &header.Context)
	} else {
		glog.V(0).Infoln("child == nil")
...
func (c *rawBridge) Lookup(header *fuse.InHeader, name string, out *fuse.EntryOut) (code fuse.Status) {
	...
	glog.V(0).Infoln("rawBridge  Lookup")
...
func (c *rawBridge) Open(input *fuse.OpenIn, out *fuse.OpenOut) (status fuse.Status) {
	...
	h, opened := inode.mount.registerFileHandle(inode, nil, f, input.Flags)
	out.OpenFlags = opened.FuseFlags
	out.Fh = h
	return fuse.OK
}
...
func (c *rawBridge) Fallocate(input *fuse.FallocateIn) (code fuse.Status) {
	...
	return n.fsInode.Fallocate(*opened.File, input.Offset, input.Length, input.Mode, &input.Context)
}
...
func (c *rawBridge) Mkdir(input *fuse.MkdirIn, name string, out *fuse.EntryOut) (code fuse.Status) {
	...
	if code.Ok() {
		c.childLookup(out, child, &input.Context)
		code = child.Fileinfo().GetAttr((*fuse.Attr)(&out.Attr))
	}
	return code
}
...
func (c *rawBridge) Write(input *fuse.WriteIn, data []byte) (written uint32, code fuse.Status) {
	...
	size := len(data)
	nowlen := len(opened.temp)
	if off <= nowlen {
		opened.temp = append(opened.temp[:off], data...)
	} else {
		arrlen := off - nowlen
		temp := make([]byte, arrlen)
		opened.temp = append(opened.temp, temp...)
		opened.temp = append(opened.temp, data...)
	}
...
func (c *rawBridge) Read(input *fuse.ReadIn, buf []byte) (fuse.ReadResult, fuse.Status) {
	...
	return node.Node().Read(node.fileinfo, buf, int64(input.Offset), int64(input.Size), &input.Context)
}
...
func (c *rawBridge) Flush(input *fuse.FlushIn) fuse.Status {
	...
	if opened != nil {
		if opened.OpenFlags&fuse.O_ANYWRITE != 0 {
			return node.Node().Fsync(opened)
		}
		return node.Fileinfo().Flush()
	}
	return fuse.OK
}
...
func (c *rawBridge) Fsync(input *fuse.FsyncIn) fuse.Status {
	...
	if opened != nil {
		//return inode.fsInode.Fsync(int(input.FsyncFlags))
		return inode.Node().Fsync(opened)
	}
