...
func (ms *Server) handleRequest(req *request) Status {
	...
	if req.status.Ok() && ms.opts.Debug {
		glog.V(0).Infoln(req.InputDebug())
	}

	if req.status.Ok() && req.handler.Func == nil {
		glog.V(4).Infoln("Unimplemented opcode %v", operationName(req.inHeader.Opcode))
		req.status = ENOSYS
	}
	...
	errNo := ms.write(req)
	if errNo != 0 {
		glog.V(4).Infoln("writer: Write/Writev failed, err: %v. opcode: %v",
	...
	isoperr := false
	if !req.status.Ok() {
		glog.V(1).Infoln("OP:", opname, " TIME:", time.Since(starttime), " SIZE:", size, " error:", req.status)
		isoperr = true
	}
	if isoperr && req.status == ENOENT {
		ms.TpsStats.push("ERROR_404", starttime.UnixNano(), 0)
		isoperr = false
	}
	ms.returnRequest(req)
	ms.TpsStats.push(opname, starttime.UnixNano(), size)
	if isoperr {
		ms.TpsStats.push("ERROR", starttime.UnixNano(), 0)
	}
	if ms.opts.Debug {
		glog.V(0).Infoln("OP:", opname, " TIME:", time.Since(starttime), " SIZE:", size)
	}

	glog.V(4).Infoln("handleRequest done:", errNo)
	return Status(errNo)
}
...
func (ms *Server) write(req *request) Status {
	...
	//glog.V(0).Infoln("write request", req.flatDataSize())
	...
	if ms.opts.Debug {
		glog.V(0).Infof(req.OutputDebug())
		//	log.Println(req.OutputDebug())
	}
	//glog.V(0).Infoln("req.OutputDebug:", req.OutputDebug())
	if header == nil {
		return OK
	}
...
func (ms *Server) InodeNotify(node uint64, off int64, length int64) Status {
	...
	if ms.opts.Debug {
		glog.V(0).Infof("Response: INODE_NOTIFY", result)
	}
	return result
}
...
func (ms *Server) DeleteNotify(parent uint64, child uint64, name string) Status {
	...
	if ms.opts.Debug {
		glog.V(0).Infof("Response: DELETE_NOTIFY: %v", result)
...
type FuseIostat struct {
	Ops map[string]*Optime
	sync.RWMutex
}
...
func (o *Optime) push(starttime int64, size int64) {
	o.Lock()
	defer o.Unlock()
	o.TotalSum++
	o.TotalSize += size
	o.TotalTime += (time.Now().UnixNano() - starttime)
}

func (o *Optime) reset() *Optime {
	res := &Optime{}
	o.RLock()
	defer o.RUnlock()
	res.TotalTime = o.TotalTime
	res.TotalSum = o.TotalSum
	res.TotalSize = o.TotalSize
	o.TotalSum  = 0
	o.TotalTime = 0
	o.TotalSize = 0
	return res
...
func (f *FuseIostat) push(optype string, starttime int64, size int64) {
	glog.V(4).Infoln(optype, starttime, size)
	f.RLock()
	if v, ok := f.Ops[optype]; ok {
		f.RUnlock()
		v.push(starttime, size)
		return
	}
	f.RUnlock()
...
//
func (f *FuseIostat) GetStatic() string {
	res := ""
	tempmap := make(map[string]*Optime)
	f.RLock()
	for k, v := range f.Ops {
		tempmap[k] = v.reset()
	}
	...
	for k, v := range tempmap {
		v.RLock()
		res += fmt.Sprintf("%s,%d,%d,%d|", k, v.TotalSize, v.TotalTime, v.TotalSum)
		v.RUnlock()
	}
	res += "\n"
	return res
